...
for i, rp in enumerate(REGION_PAIRS):
    ...
    for jj in range(jPECC[rp].shape[2]):
        for ii, kk in enumerate(range(asy_tb, jPECC[rp].shape[0] - asy_tb)):
            ...
            asym[rp][jj, ii] = np.mean(arr_slice[:-asy_tb] - arr_slice[asy_tb:])
...
ax1.imshow(np.flipud(np.mean(jPECC['Hipp-PPC'], axis=2).T), vmin=0.2, vmax=0.6, cmap='inferno',
...
ax2.imshow(np.flipud(np.mean(jPECC['Hipp-Thal'], axis=2).T), vmin=0.2, vmax=0.6, cmap='inferno',
           extent=[time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2,
                   time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2])
ax2.invert_xaxis()
...
plt.savefig(join(fig_path, 'jPECC_Hipp.pdf'))
...
ax1.imshow(np.flipud(np.mean(jPECC['M2-ORB'], axis=2)), vmin=0.2, vmax=0.6, cmap='inferno',
           extent=[time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2,
                   time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2])
ax1.invert_xaxis()
ax1.plot([0, 0], [-1, 2], color='white')
...
ax2.imshow(np.flipud(np.mean(jPECC['ORB-Amyg'], axis=2)), vmin=0.2, vmax=0.6, cmap='inferno',
           extent=[time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2,
                   time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2])
ax2.invert_xaxis()
ax2.plot([0, 0], [-1, 2], color='white')
...
ax1.plot(time_asy, np.mean(asym['Hipp-PPC'], axis=0), color=colors['M2'], label='M2')
...
ax1.plot(time_asy, np.mean(asym['Hipp-Thal'], axis=0), color=colors['Amyg'], label='Amygdala')
ax1.fill_between(time_asy,
                 np.mean(asym['Hipp-Thal'], axis=0)-(np.std(asym['Hipp-Thal'], axis=0)/np.sqrt(asym['Hipp-Thal'].shape[0])),
                 np.mean(asym['Hipp-Thal'], axis=0)+(np.std(asym['Hipp-Thal'], axis=0)/np.sqrt(asym['Hipp-Thal'].shape[0])),
                 alpha=0.2, color=colors['Amyg'])

ax1.plot(time_asy, np.mean(asym['PPC-Thal'], axis=0), color=colors['Amyg'], label='Amygdala')
ax1.fill_between(time_asy,
...
ax1.set(ylabel='jPECC asymmetry', xlabel='Time from stim. onset (s)', title='Medial prefrontal cortex',
        ylim=[-0.2, 0.2], xlim=[-1, 2], yticks=np.arange(-0.2, 0.21, 0.1))
...
ax2.fill_between(time_asy,
                 ...
                 np.mean(asym['ORB-Amyg'], axis=0)+(np.std(asym['ORB-Amyg'], axis=0)/np.sqrt(asym['ORB-Amyg'].shape[0])),
                 alpha=0.2, color=colors['Amyg'])
leg = ax2.legend(prop={'size': 5}, frameon=True)
leg.get_frame().set_linewidth(0)
...
ax1.set(xlabel='Time (s)', ylabel='Canonical correlation \n over baseline (r)', xlim=[-1, 2], ylim=[-0.3, 0.3],
        yticks=np.arange(-0.3, 0.31, 0.1), title='Medial prefrontal cortex')
...
ax2.set(xlabel='Time (s)', ylabel='Canonical correlation \n over baseline (r)', xlim=[-1, 2], ylim=[-0.3, 0.3],
        yticks=np.arange(-0.3, 0.31, 0.1), title='Orbitofrontal cortex')
leg_handles, _ = ax2.get_legend_handles_labels()
leg_labels = ['M2', 'Amygdala']
leg = ax2.legend(leg_handles, leg_labels, prop={'size': 5}, loc='upper left')
leg.get_frame().set_linewidth(0)
