...
def Viewer(eid, video_type, trial_range, save_vids_here, save_video=True, eye_zoom=False):
    '''
...
#    dtype_type = [x['dataset_type'] for x in a]
#    assert all([i in dtype_type for i in dataset_types]
...
#               ), 'For this eid, not all data available'
    ...
    try:
        ...
        return
    print(video_path)
#    video_path = Path('/home/mic/paw3/full_vids/cde63527-7f5a-4cc3-8ac2-215d82e7da26_iblrig_leftCamera.raw.mp4')
...
#    points = np.unique(['_'.join(x.split('_')[:-1]) for x in cam.keys()])
    ...
    try:
        ...
        print('prq??', os.path.exists(dlc_path))
        print(dlc_path)
