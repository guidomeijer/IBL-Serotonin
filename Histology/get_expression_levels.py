...
for i, subject in enumerate(subjects):
    ...
    all_rel_fluo = np.empty(np.arange(AP_EXT[0], AP_EXT[1]-100, -100).shape[0])
    for j, ap in enumerate(np.arange(AP_EXT[0], AP_EXT[1]-100, -100)):
        ...
        all_rel_fluo[j] = (np.percentile(slice_im[np.isin(slice_regions, ['DR', 'PAG'])], 95)
                           / np.percentile(slice_im[np.isin(slice_regions, ['PRNr'])], 95))
    rel_fluo = np.mean(all_rel_fluo)

    # Plot figures
    f, (ax1, ax2) = plt.subplots(1, 2, dpi=400)
    gr_hist.plot_cslice(-4500/1e6, ax=ax1)
    ...
    plot_regions[np.isin(slice_regions, ['PRNr'])] = 2
