...
sns.lineplot(x='time', y='pca_dist_spont', ax=ax1, legend=None, ci=68, hue='sert-cre',
             data=pca_df, hue_order=[0, 1], palette=[colors['wt'], colors['sert']])
ax1.set(xlim=[-1, 4], xlabel='Time (s)', ylabel='PCA trajectory distance\nfrom baseline (a.u.)',
        xticks=[0, 1, 2, 3, 4], ylim=[5, 15], yticks=[5, 10, 15])
ax1.legend(title='', frameon=False, bbox_to_anchor=(0.55, 0.6))

sns.lineplot(x='time', y='pop_dist_opto', ax=ax2, legend=None, ci=68, hue='sert-cre',
             data=pca_df, hue_order=[0, 1], palette=[colors['wt'], colors['sert']])
ax2.set(xlim=[-1, 4], xlabel='Time (s)', ylabel='Pop. activity distance\nfrom baseline (a.u.)',
        xticks=[0, 1, 2, 3, 4], ylim=[5, 50], yticks=[5, 10, 15])
ax2.legend(title='', frameon=False, bbox_to_anchor=(0.55, 0.6))

plt.tight_layout()
sns.despine(trim=True)

plt.savefig(join(fig_path, 'PCA_spont_all_neurons.pdf'))
