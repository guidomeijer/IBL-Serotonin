...
for i, eid in enumerate(eids):
    ...
    # Select trials
    trials = trials[trials['signed_contrast'].abs() >= MIN_CONTRAST].reset_index()
    ...
    for p, probe in enumerate(spikes.keys()):
        ...
        for r, region in enumerate(np.unique(clusters[probe]['acronym'])):
            ...
            times = np.column_stack(((trials['goCue_times'] - PRE_TIME), (trials['stimOn_times'] + POST_TIME)))
            ...
            # Decode
            acc_block_on = classify(population_activity[block_on_trials, :], trials.loc[block_on_trials, 'stim_side'].values,
                                    classifier, cross_validation=cv)[0]
            acc_block_off = classify(population_activity[block_off_trials, :], trials.loc[block_off_trials, 'stim_side'].values,
                               classifier, cross_validation=cv)[0]
    ...
    results_df.to_csv(join(save_path, f'lda_decoding_stim_{PRE_TIME}_{POST_TIME}.csv'))
results_df.to_csv(join(save_path, f'lda_decoding_stim_{PRE_TIME}_{POST_TIME}.csv'))
