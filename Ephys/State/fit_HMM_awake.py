...
if OVERWRITE:
    ...
    hmm_df = pd.read_csv(join(save_path, 'hmm.csv'))
    rec = rec[~rec['pid'].isin(hmm_df['pid'])]
...
for i in rec.index.values:
    ...
    for r, region in enumerate(np.unique(clusters['high_level_region'])):
        ...
        peth, binned_spikes = calculate_peths(spikes.times, spikes.clusters, clusters_in_region, opto_train_times,
                                              ...
                                              smoothing=SMOOTHING, return_fr=False)
        ...
        for j, s in enumerate(N_STATES):
            ...
            for k, (train_index, test_index) in enumerate(kf.split(trial_data)):
                ...
                this_ll[k] = simple_hmm.log_likelihood(list(np.array(trial_data)[test_index]))

            # Get average log-likelihood over folds
        ...
        """
        posterior = simple_hmm.filter(trial_data[test_index[0]])
