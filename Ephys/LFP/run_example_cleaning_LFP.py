...
"""
lfp, time = load_lfp(eid, PROBE, time_start=1000, time_end=0,
                     relative_to='end', one=one)

# Load in clean LFP
cleaned_lfp = np.load(join(save_path, f'{SUBJECT}_{DATE}_{PROBE}_cleaned_lfp.npy'))
cleaned_time = np.load(join(save_path, f'{SUBJECT}_{DATE}_{PROBE}_timestamps.npy'))
...
plt.savefig(join(fig_path, 'Example_LFP_cleaning'), dpi=300)
