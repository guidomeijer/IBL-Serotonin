...
# Load in data
corr_df = pd.read_csv(join(save_path, f'combined_region_corr_{BIN_SIZE}.csv'))

# Remove nans
corr_df = corr_df[~np.isnan(corr_df['r'])]

# Min neuron criterium
corr_df = corr_df[corr_df['n_region_1'] >= MIN_NEURONS]
corr_df = corr_df[corr_df['n_region_2'] >= MIN_NEURONS]
...
for r1, region_1 in enumerate(all_regions[:-1]):
    for r2, region_2 in enumerate(all_regions[r1+1:]):
        ...
        plt.tight_layout()
        plt.savefig(join(fig_path, 'Ephys', 'Correlations', 'CombinedRegions',
...
plt.savefig(join(fig_path, 'Ephys', 'Correlations', 'CombinedRegions', 'Correlation_Graph.jpg'),
            dpi=600)
...
sns.lineplot(x='time', y='r_baseline', data=slice_df, hue='region_pair',
             ...
             palette=[colors['OFC'], colors['PPC']], legend='brief', errorbar='se')
...
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(fig_path, 'Ephys', 'Correlations', 'CombinedRegions', 'Hippocampus_Sensory_Frontal.jpg'),
            dpi=600)
"""
