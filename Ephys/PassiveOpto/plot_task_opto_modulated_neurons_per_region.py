...
# Merge dataframes
merged_df = pd.merge(task_neurons, light_neurons, on=['neuron_id', 'subject', 'date', 'probe',
                                                      'eid', 'region'])
...
merged_df['full_region'] = combine_regions(merged_df['region'])
...
mod_neurons = merged_df[(merged_df['sert-cre'] == 1) & (merged_df['modulated'] == 1)]
mod_neurons = mod_neurons.groupby('full_region').filter(lambda x: len(x) >= MIN_MOD_NEURONS)

# Add enhanced and suppressed
merged_df['enhanced'] = merged_df['modulated'] & (merged_df['opto_mod_roc'] > 0)
merged_df['suppressed'] = merged_df['modulated'] & (merged_df['opto_mod_roc'] < 0)

# Calculate summary statistics
summary_df = merged_df[merged_df['sert-cre'] == 1].groupby(['full_region']).sum()
summary_df['n_neurons'] = merged_df[merged_df['sert-cre'] == 1].groupby(['full_region']).size()
summary_df['modulation_index'] = merged_df[merged_df['sert-cre'] == 1].groupby(['full_region']).mean()['opto_mod_roc']
summary_df = summary_df.reset_index()
summary_df['perc_enh_late'] =  (summary_df['enhanced'] / summary_df['n_neurons']) * 100
summary_df['perc_supp_late'] =  (summary_df['suppressed'] / summary_df['n_neurons']) * 100
...
summary_no_df = merged_df[merged_df['sert-cre'] == 0].groupby(['full_region']).sum()
summary_no_df['n_neurons'] = merged_df[merged_df['sert-cre'] == 0].groupby(['full_region']).size()
...
per_mouse_df = merged_df[merged_df['sert-cre'] == 1].groupby(['full_region', 'subject']).sum()
per_mouse_df['n_neurons'] = merged_df[merged_df['sert-cre'] == 1].groupby(['full_region', 'subject']).size()
...
plt.savefig(join(fig_path, 'perc_optotask_modulated_neurons_per_region_pooled.pdf'))
...
plt.savefig(join(fig_path, 'perc_optotask_modulated_neurons_per_region.pdf'))
...
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(fig_path, 'optotask_modulation_per_region.pdf'))
...
ORDER = mod_neurons.groupby('full_region').mean()['opto_mod_roc'].sort_values(ascending=False).reset_index()['full_region']

f, ax1 = plt.subplots(1, 1, figsize=(3, 2), dpi=dpi)
sns.boxplot(x='opto_mod_roc', y='full_region', ax=ax1, data=mod_neurons, showmeans=True,
            order=ORDER, meanprops={"marker": "|", "markeredgecolor": "black", "markersize": "7"},
            fliersize=0, **PROPS)
sns.stripplot(x='opto_mod_roc', y='full_region', ax=ax1, data=mod_neurons, order=ORDER,
...
plt.tight_layout()
sns.despine(trim=True)
plt.savefig(join(fig_path, 'optotask_modulation_per_neuron_per_region.pdf'))
