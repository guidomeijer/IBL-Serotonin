...
file_name = 'CCA_pre_post_opto.pickle'
...
for i, eid in enumerate(np.unique(rec['eid'])):
    ...
    for r, reg_pair in enumerate(REGION_PAIRS):
        ...
        if (region_1 in pca_opto.keys()) & (region_2 in pca_opto.keys()):
            ...
            r_pre = np.empty((2, psth_opto['tscale'].shape[0], N_MODES))
            reg_1_pre = np.concatenate(pca_opto[region_1][
                1::2, :, ((psth_opto['tscale'] >= -PRE_WIN[0]) & (psth_opto['tscale'] <= -PRE_WIN[1]))], axis=1).T
            reg_2_pre = np.concatenate(pca_opto[region_2][
                1::2, :, ((psth_opto['tscale'] >= -PRE_WIN[0]) & (psth_opto['tscale'] <= -PRE_WIN[1]))], axis=1).T
            cca.fit(reg_1_pre, reg_2_pre)
            for it, tb in enumerate(psth_opto['tscale']):
                ...
                for im in range(N_MODES):
                    r_pre[0, it, im] = pearsonr(x.T[im, :], y.T[im, :])[0]

            # Fit to even trials and project odd trials
            reg_1_pre = np.concatenate(pca_opto[region_1][
                ::2, :, ((psth_opto['tscale'] >= -PRE_WIN[0]) & (psth_opto['tscale'] <= -PRE_WIN[1]))], axis=1).T
            reg_2_pre = np.concatenate(pca_opto[region_2][
                ::2, :, ((psth_opto['tscale'] >= -PRE_WIN[0]) & (psth_opto['tscale'] <= -PRE_WIN[1]))], axis=1).T
            cca.fit(reg_1_pre, reg_2_pre)
            for it, tb in enumerate(psth_opto['tscale']):
                ...
                for im in range(N_MODES):
                    r_pre[1, it, im] = pearsonr(x.T[im, :], y.T[im, :])[0]

            # Take mean of the two folds (odd and even)
            r_pre = np.mean(r_pre, axis=0)

            # POST OPTO WINDOW
            # Fit to odd trials and project even trials
            r_post = np.empty((2, psth_opto['tscale'].shape[0], N_MODES))
            reg_1_post = np.concatenate(pca_opto[region_1][
                1::2, :, ((psth_opto['tscale'] >= POST_WIN[0]) & (psth_opto['tscale'] <= POST_WIN[1]))], axis=1).T
            reg_2_post = np.concatenate(pca_opto[region_2][
                1::2, :, ((psth_opto['tscale'] >= POST_WIN[0]) & (psth_opto['tscale'] <= POST_WIN[1]))], axis=1).T
            cca.fit(reg_1_post, reg_2_post)
            for it, tb in enumerate(psth_opto['tscale']):
                x, y = cca.transform(pca_opto[region_1][::2, :, it], pca_opto[region_2][::2, :, it])
                for im in range(N_MODES):
                    r_post[0, it, im] = pearsonr(x.T[im, :], y.T[im, :])[0]

            # Fit to even trials and project odd trials
            reg_1_post = np.concatenate(pca_opto[region_1][
                ::2, :, ((psth_opto['tscale'] >= POST_WIN[0]) & (psth_opto['tscale'] <= POST_WIN[1]))], axis=1).T
            reg_2_post = np.concatenate(pca_opto[region_2][
                ::2, :, ((psth_opto['tscale'] >= POST_WIN[0]) & (psth_opto['tscale'] <= POST_WIN[1]))], axis=1).T
            cca.fit(reg_1_post, reg_2_post)
            for it, tb in enumerate(psth_opto['tscale']):
                x, y = cca.transform(pca_opto[region_1][1::2, :, it], pca_opto[region_2][1::2, :, it])
                for im in range(N_MODES):
                    r_post[1, it, im] = pearsonr(x.T[im, :], y.T[im, :])[0]

            # Take mean of the two folds (odd and even)
            r_post = np.mean(r_post, axis=0)
            ...
            cca_df = pd.concat((cca_df, pd.DataFrame(index=[cca_df.shape[0]], data={
                ...
                'region_pair': f'{region_1}-{region_2}', 'r_pre': [r_pre], 'r_post': [r_post],
                'n_modes': N_MODES, 'time': [psth_opto['tscale']]})))
