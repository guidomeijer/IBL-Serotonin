...
cca_df = cca_df.melt(ignore_index=False).reset_index()
...
ax1.set(ylabel='Time from stimulation onset (s)', xlabel='Delay (s)',
        title='mPFC vs M2', ylim=[-1, 3],
        xticks=[time_asy[0], 0, time_asy[-1]])

ax2.imshow(np.flipud(np.mean(jPECC['mPFC-Amyg'], axis=2)), vmin=-0.5, vmax=0.5, cmap='icefire',
           interpolation='nearest', aspect='auto',
           extent=[time_asy[0], time_asy[-1],
                   time_ax[0] - np.mean(np.diff(time_ax))/2, time_ax[-1] + np.mean(np.diff(time_ax))/2])
ax2.invert_xaxis()
ax2.plot([0, 0], [-1, 3], color='white', ls='--', lw=0.5)
ax2.set(ylabel='Time from stimulation onset (s)', xlabel='Delay (s)', title='mPFC vs Amygdala', ylim=[-1, 3],
        xticks=[time_asy[0], 0, time_asy[-1]])
