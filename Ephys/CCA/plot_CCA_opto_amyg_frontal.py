...
sns.lineplot(x='time', y='r_baseline', hue='region_pair', data=cca_df[cca_df['sert-cre'] == 1], ax=ax1,
             palette='tab10')
ax1.set(ylabel='Population correlation (r)', xlabel='Time (s)')
ax1.legend(frameon=False)

sns.lineplot(x='time', y='r_baseline', hue='region_pair', data=cca_df[cca_df['sert-cre'] == 0], ax=ax2,
             palette='tab10')
...
sns.lineplot(x='time', y='r_baseline', data=cca_df[(cca_df['sert-cre'] == 1) & (cca_df['region_pair'] == 'M2-mPFC')], ax=ax1,
             palette='tab10')
sns.lineplot(x='time', y='r_baseline', data=cca_df[(cca_df['sert-cre'] == 1) & (cca_df['region_pair'] == 'M2-ORB')], ax=ax1,
             palette='tab10')
sns.lineplot(x='time', y='r_baseline', data=cca_df[(cca_df['sert-cre'] == 1) & (cca_df['region_pair'] == 'mPFC-ORB')], ax=ax1,
             palette='tab10')
ax1.plot(ax1.get_xlim(), [0, 0], ls='--', color='grey')

sns.lineplot(x='time', y='r_baseline', data=cca_df[(cca_df['sert-cre'] == 1) & (cca_df['region_pair'] == 'Amyg-mPFC')], ax=ax2,
             palette='tab10')
ax2.plot(ax2.get_xlim(), [0, 0], ls='--', color='grey')