...
switches_prevaction = pd.DataFrame()
switches_stimside = pd.DataFrame()
for i, nickname in enumerate(subjects['subject']):
    ...
    results_df = results_df.append(pd.DataFrame(data={'tau': [1/param_prevaction[0], 1/param_prevaction[1]],
                                                      ...
                                                      'subject': nickname, 'model': 'prev-action'}))
        ...
        for t, trans in enumerate(transitions):
            if trans >= PRE_TRIALS:
                if stimulated[k][trans] == 1:
                    ...
                    opto = 'no stim'
                switches_prevaction = switches_prevaction.append(pd.DataFrame(data={
    ...
    sns.lineplot(x='trial', y='prior', data=switches_prevaction[switches_prevaction['subject'] == nickname],
    ...
    priors = model.compute_signal(signal='prior', act=actions, stim=stimuli, side=stim_side)['prior']
    results_df = results_df.append(pd.DataFrame(data={'tau': [1/param_stimside[0], 1/param_stimside[1]],
                                                      'opto_stim': ['no stim', 'stim'],
                                                      'sert-cre': subjects.loc[i, 'sert-cre'],
                                                      'subject': nickname, 'model': 'stim-side'}))
        ...
        for t, trans in enumerate(transitions):
            if trans >= PRE_TRIALS:
                if stimulated[k][trans] == 1:
                    ...
                    opto = 'no stim'
                switches_stimside = switches_stimside.append(pd.DataFrame(data={
                            'prior': priors[k][trans-PRE_TRIALS:trans+POST_TRIALS],
                            'trial': np.append(np.arange(-PRE_TRIALS, 0), np.arange(0, POST_TRIALS)),
                            'change_to': prob_left[k][trans],
                            'opto': opto,
    ...
    # Plot for this animal
    f, ax1 = plt.subplots(1, 1, figsize=(10, 10))
    sns.lineplot(x='trial', y='prior', data=switches_stimside[switches_stimside['subject'] == nickname],
             hue='change_to', style='opto', palette='colorblind', ax=ax1, ci=68)
    #plt.plot([0, 0], [0, 1], color=[0.5, 0.5, 0.5], ls='--')
    handles, labels = ax1.get_legend_handles_labels()
    labels = ['', 'Change to R', 'Change to L', '', 'No stim', 'Stim']
...
sns.lineplot(x='opto_stim', y='tau', hue='sert-cre', style='subject', estimator=None,
             data=results_df[results_df['model'] == 'prev-action'], dashes=False,
             markers=['o']*int(results_df[results_df['model'] == 'prev-action'].shape[0]/2),
...
ax1.set(xlabel='', ylabel='Lenght of integration window (tau)', title='Previous actions')

block_avg_sert = switches_prevaction[switches_prevaction['sert_cre'] == 1].groupby(['subject', 'trial', 'change_to', 'opto']).mean().reset_index()
...
block_avg_wt = switches_prevaction[switches_prevaction['sert_cre'] == 0].groupby(['subject', 'trial', 'change_to', 'opto']).mean().reset_index()
...
f, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(20, 5), dpi=150)
sns.lineplot(x='opto_stim', y='tau', hue='sert-cre', style='subject', estimator=None,
             data=results_df[results_df['model'] == 'stim-side'], dashes=False,
             markers=['o']*int(results_df[results_df['model'] == 'stim-side'].shape[0]/2),
             legend='brief', palette=[colors['wt'], colors['sert']], lw=2, ms=8, ax=ax1)
handles, labels = ax1.get_legend_handles_labels()
labels = ['', 'WT', 'SERT']
ax1.legend(handles[:3], labels[:3], frameon=False, prop={'size': 20}, loc='center left', bbox_to_anchor=(1, .5))
ax1.set(xlabel='', ylabel='Lenght of integration window (tau)', title='Stimulus sides')

block_avg_sert = switches_stimside[switches_stimside['sert_cre'] == 1].groupby(['subject', 'trial', 'change_to', 'opto']).mean().reset_index()
sns.lineplot(x='trial', y='prior', data=block_avg_sert, hue='change_to', style='opto',
             palette=[colors['right'], colors['left']], units='subject', estimator=None, legend=None, ax=ax2)
#plt.plot([0, 0], [0, 1], color=[0.5, 0.5, 0.5], ls='--')
ax2.set(ylabel='Prior', xlabel='Trials relative to block switch', title='Sert-Cre', ylim=[0, 1])

block_avg_wt = switches_stimside[switches_stimside['sert_cre'] == 0].groupby(['subject', 'trial', 'change_to', 'opto']).mean().reset_index()
sns.lineplot(x='trial', y='prior', data=block_avg_wt, hue='change_to', style='opto',
             palette=[colors['right'], colors['left']], units='subject', estimator=None, ax=ax3)
#plt.plot([0, 0], [0, 1], color=[0.5, 0.5, 0.5], ls='--')
handles, labels = ax3.get_legend_handles_labels()
