...
fig_path = join(fig_path, 'Behavior', 'Models', 'BadSessions')
...
for i, nickname in enumerate(subjects['subject']):
    ...
    _, eids = behavioral_criterion(eids, return_excluded=True, one=one)
    if len(eids) == 0:
        continue
    if len(eids) > 10:
        eids = eids[:10]
...
sns.despine(trim=True)
plt.tight_layout()
plt.savefig(join(fig_path, f'exp-smoothing_opto_{STIM}'), dpi=600)
