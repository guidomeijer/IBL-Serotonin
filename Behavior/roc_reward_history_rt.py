...
for i, nickname in enumerate(subjects['subject']):
    ...
    for j, eid in enumerate(eids):
        ...
        rew_bias_opto = trials.loc[~trials['rew_bias_opto'].isnull()
                                       & (trials['reaction_times'] > RT_CUTOFF), 'rew_bias_opto'].values
        choice = trials.loc[~trials['rew_bias_opto'].isnull()
                                & (trials['reaction_times'] > RT_CUTOFF), 'choice'].values
        opto_long = roc_auc_score(choice, rew_bias_opto)

        rew_bias_opto = trials.loc[~trials['rew_bias_opto'].isnull()
                                       & (trials['reaction_times'] < RT_CUTOFF), 'rew_bias_opto'].values
        choice = trials.loc[~trials['rew_bias_opto'].isnull()
                                & (trials['reaction_times'] < RT_CUTOFF), 'choice'].values
        opto_short = roc_auc_score(choice, rew_bias_opto)

        rew_bias_opto = trials.loc[~trials['rew_bias_no_opto'].isnull()
                                       & (trials['reaction_times'] > RT_CUTOFF), 'rew_bias_no_opto'].values
        choice = trials.loc[~trials['rew_bias_no_opto'].isnull()
                                & (trials['reaction_times'] > RT_CUTOFF), 'choice'].values
        if len(np.unique(choice)) == 2:
            no_opto_long = roc_auc_score(choice, rew_bias_opto)
        else:
            no_opto_long = np.nan
        ...
        choice = trials.loc[~trials['rew_bias_no_opto'].isnull()
                                & (trials['reaction_times'] < RT_CUTOFF), 'choice'].values
        no_opto_short = roc_auc_score(choice, rew_bias_opto)
...
plt.savefig(join(fig_path, 'roc_reward_history'), dpi=300)
