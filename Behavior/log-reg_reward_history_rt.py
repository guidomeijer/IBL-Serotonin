...
for i, nickname in enumerate(subjects['subject']):
    ...
    for j, eid in enumerate(eids):
        ...
        log_reg.fit(rew_bias_opto, choice)
        opto_long = log_reg.score(rew_bias_opto, choice)
        ...
        choice = all_trials.loc[~all_trials['rew_bias_opto'].isnull()
                                & (all_trials['reaction_times'] < RT_CUTOFF), 'choice'].values
        log_reg.fit(rew_bias_opto, choice)
        opto_short = log_reg.score(rew_bias_opto, choice)
        ...
        if len(np.unique(choice)) == 2:
            log_reg.fit(rew_bias_opto, choice)
            no_opto_long = log_reg.score(rew_bias_opto, choice)
        else:
            no_opto_long = np.nan
        ...
        choice = all_trials.loc[~all_trials['rew_bias_no_opto'].isnull()
                                & (all_trials['reaction_times'] < RT_CUTOFF), 'choice'].values
        log_reg.fit(rew_bias_opto, choice)
        no_opto_short = log_reg.score(rew_bias_opto, choice)
